...
_ccs_tree_tuner_random_del(ccs_object_t o) {
	_ccs_random_tree_tuner_data_t *d = (_ccs_random_tree_tuner_data_t *)((ccs_tree_tuner_t)o)->data;
...
_ccs_serialize_bin_size_ccs_random_tree_tuner(
	...
	*cum_size += _ccs_serialize_bin_size_ccs_object_internal(
	...
	CCS_VALIDATE(_ccs_serialize_bin_size_ccs_random_tree_tuner_data(data, cum_size, opts));
	return CCS_SUCCESS;
}
...
_ccs_tree_tuner_random_tell(
	...
	for (size_t i = 0; i < num_evaluations; i++) {
		ccs_result_t error;
		CCS_VALIDATE(ccs_tree_evaluation_get_error(evaluations[i], &error));
		if (!error) {
			int discard = 0;
			UT_array *tmp;
...
#define utarray_oom() { \
	...
	CCS_RAISE(CCS_OUT_OF_MEMORY, "Out of memory to allocate array"); \
			...
			while ( (eval = (ccs_tree_evaluation_t *)utarray_next(d->old_optimums, eval)) ) {
				if (!discard) {
					ccs_comparison_t cmp;
					err = ccs_tree_evaluation_compare(evaluations[i], *eval, &cmp);
					if (err)
						discard = 1;
					else switch (cmp) {
					case CCS_EQUIVALENT:
...
static ccs_error_t
_ccs_tree_tuner_random_get_optimums(
		ccs_tree_tuner_t        tuner,
		size_t                  num_evaluations,
		ccs_tree_evaluation_t  *evaluations,
	...
	if (evaluations) {
		...
		ccs_tree_evaluation_t *eval = NULL;
		size_t index = 0;
		while ( (eval = (ccs_tree_evaluation_t *)utarray_next(d->optimums, eval)) )
...
_ccs_tree_tuner_random_get_history(
	...
	if (evaluations) {
		...
		ccs_tree_evaluation_t *eval = NULL;
		size_t index = 0;
		while ( (eval = (ccs_tree_evaluation_t *)utarray_next(d->history, eval)) )
			evaluations[index++] = *eval;
		for (size_t i = count; i <num_evaluations; i++)
			evaluations[i] = NULL;
	}
...
_ccs_tree_tuner_random_suggest(
	...
	if (count > 0) {
		...
		CCS_VALIDATE(ccs_tree_space_get_rng(d->common_data.tree_space, &rng));
		CCS_VALIDATE(ccs_rng_get(rng, &indx));
		indx = indx % count;
		ccs_tree_evaluation_t *eval =
			(ccs_tree_evaluation_t *)utarray_eltptr(d->optimums, indx);
		CCS_VALIDATE(ccs_tree_evaluation_get_configuration(*eval, configuration));
		CCS_VALIDATE(ccs_retain_object(*configuration));
	} else
		CCS_VALIDATE(_ccs_tree_tuner_random_ask(tuner, 1, configuration, NULL));
	return CCS_SUCCESS;
}

static _ccs_tree_tuner_ops_t _ccs_tree_tuner_random_ops = {
	{ &_ccs_tree_tuner_random_del,
	  &_ccs_tree_tuner_random_serialize_size,
	  &_ccs_tree_tuner_random_serialize },
	&_ccs_tree_tuner_random_ask,
	&_ccs_tree_tuner_random_tell,
...
ccs_create_random_tree_tuner(
	...
	uintptr_t mem = (uintptr_t)calloc(1, sizeof(struct _ccs_tree_tuner_s) +
	...
	_ccs_object_init(&(tun->obj), CCS_TREE_TUNER, (_ccs_object_ops_t *)&_ccs_tree_tuner_random_ops);
	...
	data->common_data.name = (const char *)(mem + sizeof(struct _ccs_tree_tuner_s) +
                                                      sizeof(struct _ccs_random_tree_tuner_data_s));
