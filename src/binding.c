	...
	ccs_datum_t  *value_ret)
{
	...
	CCS_VALIDATE(_ccs_binding_get_value_by_name(
		binding, name, found_ret, value_ret));
	return CCS_RESULT_SUCCESS;
}
...
ccs_binding_hash(ccs_binding_t binding, ccs_hash_t *hash_ret)
{
	...
	CCS_VALIDATE(ops->hash(binding, hash_ret));
	return CCS_RESULT_SUCCESS;
}
...
ccs_binding_cmp(ccs_binding_t binding, ccs_binding_t other_binding, int *cmp_ret)
{
	...
	CCS_REFUTE(
		binding->obj.type != other_binding->obj.type,
	...
	_ccs_binding_ops_t *ops = ccs_binding_get_ops(binding);
	CCS_VALIDATE(ops->cmp(binding, other_binding, cmp_ret));
	return CCS_RESULT_SUCCESS;
}
