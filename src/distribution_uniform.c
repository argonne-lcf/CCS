...
static inline size_t
_ccs_serialize_bin_size_ccs_distribution_uniform(ccs_distribution_t distribution)
{
	_ccs_distribution_uniform_data_t *data =
...
_ccs_distribution_uniform_get_bounds(
	...
	ccs_interval_t           *interval_ret)
{
	_ccs_distribution_uniform_data_t *d =
		(_ccs_distribution_uniform_data_t *)data;
	ccs_numeric_t l;
	ccs_bool_t    li;
	ccs_numeric_t u;
	...
	ccs_numeric_t            *values)
{
	...
	if (data_type == CCS_NUM_FLOAT) {
		...
		if (scale_type == CCS_LOGARITHMIC) {
			...
			for (i = 0; i < num_values; i++) {
				values[i * stride].i = gsl_rng_uniform_int(
					grng, internal_upper.i);
			}
			if (quantize)
				for (i = 0; i < num_values; i++)
					values[i * stride].i =
	...
	ccs_numeric_t            *values)
{
	...
	if (data_type == CCS_NUM_FLOAT) {
		...
		if (scale_type == CCS_LOGARITHMIC) {
			...
			for (i = 0; i < num_values; i++)
				values[i].f =
					floor(values[i].f) * quantization.f +
					lower.f;
		else
			for (i = 0; i < num_values; i++)
				values[i].f += lower.f;
		...
		if (scale_type == CCS_LOGARITHMIC) {
			...
			if (quantize)
				for (i = 0; i < num_values; i++)
					values[i].i = ((values[i].i - lower.i) /
						       quantization.i) *
							      quantization.i +
						      lower.i;
		} else {
			for (i = 0; i < num_values; i++) {
				values[i].i = gsl_rng_uniform_int(
					grng, internal_upper.i);
			}
			if (quantize)
				for (i = 0; i < num_values; i++)
					values[i].i =
						values[i].i * quantization.i +
						lower.i;
			else
				for (i = 0; i < num_values; i++)
					values[i].i += lower.i;
