...
typedef struct _ccs_user_defined_tree_tuner_data_s
	_ccs_user_defined_tree_tuner_data_t;

static ccs_error_t
_ccs_tree_tuner_user_defined_del(ccs_object_t o)
{
	_ccs_user_defined_tree_tuner_data_t *d =
		(_ccs_user_defined_tree_tuner_data_t *)((ccs_tree_tuner_t)o)
			->data;
	ccs_error_t err;
	...
	*cum_size += _ccs_serialize_bin_size_size(num_optimums);
	if (0 != history_size + num_optimums) {
		...
		if (history_size) {
			CCS_VALIDATE_ERR_GOTO(
				...
				data->vector.get_history(
					tuner, history_size, history, NULL),
				end);
		...
		if (num_optimums) {
			CCS_VALIDATE_ERR_GOTO(
				res,
				data->vector.get_optimums(
					tuner, num_optimums, optimums, NULL),
				end);
	...
	_ccs_object_serialize_options_t *opts)
{
	...
	CCS_VALIDATE(
		_ccs_serialize_bin_size(num_optimums, buffer_size, buffer));
	if (0 != history_size + num_optimums) {
		history = (ccs_tree_evaluation_t *)calloc(
			sizeof(ccs_tree_evaluation_t),
			history_size + num_optimums);
	...
	CCS_VALIDATE_ERR_GOTO(
		res,
		_ccs_serialize_bin_size(state_size, buffer_size, buffer),
		end);
	if (state_size) {
		CCS_REFUTE_ERR_GOTO(
			res,
