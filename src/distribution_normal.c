...
{
	return _ccs_serialize_bin_size_ccs_distribution_common_data(
		       &data->common_data) +
...
static inline size_t
_ccs_serialize_bin_size_ccs_distribution_normal(ccs_distribution_t distribution)
{
	_ccs_distribution_normal_data_t *data =
...
_ccs_distribution_normal_get_bounds(
	...
	ccs_interval_t           *interval_ret)
{
	_ccs_distribution_normal_data_t *d =
		(_ccs_distribution_normal_data_t *)data;
	...
	if (scale_type == CCS_LOGARITHMIC) {
		if (data_type == CCS_NUM_FLOAT) {
			...
			if (quantize) {
				l.i = quantization.i;
				u.i = (CCS_INT_MAX / quantization.i) *
				      quantization.i;
			} else {
				l.i = 1;
				u.i = CCS_INT_MAX;
		...
		if (data_type == CCS_NUM_FLOAT) {
			...
			if (quantize) {
				l.i = (CCS_INT_MIN / quantization.i) *
				      quantization.i;
				u.i = (CCS_INT_MAX / quantization.i) *
				      quantization.i;
			} else {
	...
	ccs_numeric_t         *values)
{
	...
	if (scale_type == CCS_LOGARITHMIC) {
		...
		for (i = 0; i < num_values; i++)
			do {
				values[i].f =
					gsl_ran_gaussian(grng, sigma) + mu;
			} while (CCS_UNLIKELY(
				values[i].f - q > (ccs_float_t)CCS_INT_MAX ||
				values[i].f + q < (ccs_float_t)CCS_INT_MIN));
	if (quantize) {
		ccs_float_t rquantization = 1.0 / quantization;
		for (i = 0; i < num_values; i++)
			values[i].i =
				(ccs_int_t)round(values[i].f * rquantization) *
				quantization;
	} else
		for (i = 0; i < num_values; i++)
		...
		for (i = 0; i < num_values; i++)
			do {
				...
				values[i * stride].f - q >
					(ccs_float_t)CCS_INT_MAX ||
				values[i * stride].f + q <
					(ccs_float_t)CCS_INT_MIN));
	if (quantize) {
		ccs_float_t rquantization = 1.0 / quantization;
		for (i = 0; i < num_values; i++)
			values[i * stride].i =
				(ccs_int_t)round(
					values[i * stride].f * rquantization) *
				quantization;
	} else
		for (i = 0; i < num_values; i++)
