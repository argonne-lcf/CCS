...
#define CCS_RAISE(error, ...)                                                  \
	do {                                                                   \
		CCS_CREATE_ERROR(error, __VA_ARGS__);                          \
		return error;                                                  \
	} while (0)

#define CCS_RAISE_ERR_GOTO(err, error, label, ...)                             \
	do {                                                                   \
		CCS_CREATE_ERROR(error, __VA_ARGS__);                          \
		err = error;                                                   \
		goto label;                                                    \
...
#define CCS_REFUTE_MSG(cond, error, ...)                                       \
	do {                                                                   \
		if (CCS_UNLIKELY(cond))                                        \
			CCS_RAISE(error, __VA_ARGS__);                         \
	} while (0)

#define CCS_REFUTE_MSG_ERR_GOTO(err, cond, error, label, ...)                  \
	do {                                                                   \
		if (CCS_UNLIKELY(cond))                                        \
...
#define CCS_CHECK_PTR(p)                                                       \
	CCS_REFUTE_MSG(                                                        \
		...
		"NULL pointer supplied '%s'", #p);

#define CCS_CHECK_ARY(c, a)                                                    \
	CCS_REFUTE_MSG(                                                        \
		(c > 0) && !(a), CCS_RESULT_ERROR_INVALID_VALUE,               \
		"Invalid array '%s' == %p of size '%s' == %zu supplied", #a,   \
		a, #c, c)

#define CCS_REFUTE(cond, error)                                                \
	CCS_REFUTE_MSG(                                                        \
		cond, error, "%s: Error condition '%s' was verified", #error,  \
		#cond)

#define CCS_REFUTE_ERR_GOTO(err, cond, error, label)                           \
	CCS_REFUTE_MSG_ERR_GOTO(                                               \
		err, cond, error, label,                                       \
		"%s: Error condition '%s' was verified", #error, #cond)

#define CCS_VALIDATE_ERR_GOTO(err, cmd, label)                                 \
	do {                                                                   \
		err = (cmd);                                                   \
		if (CCS_UNLIKELY(err < CCS_RESULT_SUCCESS)) {                  \
			CCS_ADD_STACK_ELEM();                                  \
			goto label;                                            \
		}                                                              \
	} while (0)

#define CCS_VALIDATE_ERR(err, cmd)                                             \
	do {                                                                   \
		err = (cmd);                                                   \
		if (CCS_UNLIKELY(err < CCS_RESULT_SUCCESS)) {                  \
			CCS_ADD_STACK_ELEM();                                  \
...
#define CCS_SERIALIZATION_API_VERSION               ((CCS_SERIALIZATION_API_VERSION_TYPE)1)
#define CCS_SERIALIZATION_API_VERSION_SERIALIZE_BIN _ccs_serialize_bin_uint32
#define CCS_SERIALIZATION_API_VERSION_SERIALIZE_SIZE_BIN                       \
	_ccs_serialize_bin_size_uint32
#define CCS_SERIALIZATION_API_VERSION_DESERIALIZE_BIN                          \
	_ccs_deserialize_bin_uint32

typedef void *ccs_user_data_t;

/* "CCS" */
#define CCS_MAGIC_TAG                                                          \
	{                                                                      \
		0x43, 0x43, 0x53, 0x00                                         \
	}
...
{
	CCS_REFUTE(*buffer_size < 4, CCS_RESULT_ERROR_NOT_ENOUGH_DATA);
	memcpy(*buffer, tag, 4);
	*buffer += 4;
	*buffer_size -= 4;
...
_ccs_serialize_bin_size_magic_tag(const char *tag)
{
	return strlen(tag) + 1;
}
...
ccs_bswap_uint16(uint16_t x)
{
	return (((x & 0xff00u) >> 8) | ((x & 0x00ffu) << 8));
}

static inline uint32_t
ccs_bswap_uint32(uint32_t x)
{
...
static inline uint8_t
ccs_zigzag_encode_int8(int8_t x)
{
	return ((uint8_t)x << 1) ^ -((uint8_t)x >> 7);
}

static inline uint16_t
ccs_zigzag_encode_int16(int16_t x)
{
	return ((uint16_t)x << 1) ^ -((uint16_t)x >> 15);
}

static inline uint32_t
ccs_zigzag_encode_int32(int32_t x)
{
	return ((uint32_t)x << 1) ^ -((uint32_t)x >> 31);
}

static inline uint64_t
ccs_zigzag_encode_int64(int64_t x)
{
	return ((uint64_t)x << 1) ^ -((uint64_t)x >> 63);
}

static inline int8_t
ccs_zigzag_decode_int8(uint8_t x)
{
	return (int8_t)((x >> 1) ^ -(x & 0x1));
}

static inline int16_t
ccs_zigzag_decode_int16(uint16_t x)
{
	return (int16_t)((x >> 1) ^ -(x & 0x1));
}

static inline int32_t
ccs_zigzag_decode_int32(uint32_t x)
{
	return (int32_t)((x >> 1) ^ -(x & 0x1));
}

static inline int64_t
ccs_zigzag_decode_int64(uint64_t x)
{
...
#define CCS_SWAP_CONVERT(TYPE, MAPPED_NAME, MAPPED_TYPE)                       \
	do {                                                                   \
		static_assert(                                                 \
			sizeof(MAPPED_TYPE) == sizeof(TYPE),                   \
...
#define CCS_COMPRESSED_SERIALIZER_SIGNED(NAME, TYPE, MAPPED_NAME, MAPPED_TYPE) \
	...
	{                                                                      \
		return _ccs_serialize_bin_u##MAPPED_NAME(                      \
			ccs_zigzag_encode_##MAPPED_NAME(x), buffer_size,       \
			buffer);                                               \
	}                                                                      \
	static inline size_t _ccs_serialize_bin_size_##NAME(TYPE x)            \
	{                                                                      \
...
#define CCS_COMPRESSED_DESERIALIZER_SIGNED(                                    \
	...
	{                                                                      \
		u##MAPPED_TYPE v;                                              \
		CCS_VALIDATE(_ccs_deserialize_bin_u##MAPPED_NAME(              \
			&v, buffer_size, buffer));                             \
...
#define CCS_COMPRESSED_SERIALIZER_POINTER(                                     \
	...
	{                                                                      \
		return _ccs_serialize_bin_uint64(                              \
			(uint64_t)(uintptr_t)x, buffer_size, buffer);          \
	}                                                                      \
	static inline size_t _ccs_serialize_bin_size_##NAME(TYPE x)            \
	{                                                                      \
...
#define CCS_COMPRESSED_DESERIALIZER_POINTER(                                   \
	...
	{                                                                      \
		uint64_t v;                                                    \
		CCS_VALIDATE(                                                  \
			_ccs_deserialize_bin_uint64(&v, buffer_size, buffer)); \
...
#define CCS_CONVERTER_UNCOMPRESSED(NAME, TYPE, SIZE)                           \
	CCS_CONVERTER_TYPE(                                                    \
		uncompressed_##NAME, TYPE, uint##SIZE, uint##SIZE##_t)
...
#define CCS_CONVERTER_COMPRESSED_SIGNED_TYPE(                                  \
	NAME, TYPE, MAPPED_NAME, MAPPED_TYPE)                                  \
...
#define CCS_CONVERTER_COMPRESSED_SIGNED(NAME, TYPE, SIZE)                      \
	CCS_CONVERTER_COMPRESSED_SIGNED_TYPE(                                  \
		NAME, TYPE, int##SIZE, int##SIZE##_t)

#define CCS_CONVERTER_COMPRESSED_POINTER_TYPE(                                 \
	NAME, TYPE, MAPPED_NAME, MAPPED_TYPE)                                  \
	CCS_COMPRESSED_SERIALIZER_POINTER(                                     \
		NAME, TYPE, MAPPED_NAME, MAPPED_TYPE)                          \
	CCS_COMPRESSED_DESERIALIZER_POINTER(                                   \
		NAME, TYPE, MAPPED_NAME, MAPPED_TYPE)
...
CCS_CONVERTER(ccs_parameter_type, ccs_parameter_type_t, 32)
...
CCS_CONVERTER_COMPRESSED(ccs_parameter_type, ccs_parameter_type_t, 32)
...
_ccs_serialize_bin_string(const char *str, size_t *buffer_size, char **buffer)
{
	...
	CCS_REFUTE(*buffer_size < sz, CCS_RESULT_ERROR_NOT_ENOUGH_DATA);
...
_ccs_serialize_bin_ccs_blob(_ccs_blob_t *b, size_t *buffer_size, char **buffer)
{
	...
	CCS_REFUTE(*buffer_size < b->sz, CCS_RESULT_ERROR_NOT_ENOUGH_DATA);
	...
	return _ccs_serialize_bin_size_ccs_numeric_type(interval->type) +
	       (interval->type == CCS_NUMERIC_TYPE_FLOAT ?
			...
			_ccs_serialize_bin_size_ccs_int(interval->lower.i) +
				_ccs_serialize_bin_size_ccs_int(
					interval->upper.i)) +
	...
	char                **buffer)
{
	...
	if (interval->type == CCS_NUMERIC_TYPE_FLOAT) {
		...
		CCS_VALIDATE(_ccs_serialize_bin_ccs_float(
			interval->upper.f, buffer_size, buffer));
	} else {
		CCS_VALIDATE(_ccs_serialize_bin_ccs_int(
			interval->lower.i, buffer_size, buffer));
		CCS_VALIDATE(_ccs_serialize_bin_ccs_int(
			interval->upper.i, buffer_size, buffer));
	}
	CCS_VALIDATE(_ccs_serialize_bin_ccs_bool(
		interval->lower_included, buffer_size, buffer));
	CCS_VALIDATE(_ccs_serialize_bin_ccs_bool(
		interval->upper_included, buffer_size, buffer));
	return CCS_RESULT_SUCCESS;
}
	...
	char      **buffer)
{
	...
	default:
		CCS_RAISE(
			...
			"Unsupported datum type: %d", datum.type);
	}
	return CCS_RESULT_SUCCESS;
}
	...
	const char **buffer)
{
	...
	default:
		*datum = ccs_none;
		CCS_RAISE(
			CCS_RESULT_ERROR_INVALID_TYPE,
			"Unsupported datum type: %d", type);
	}
	return CCS_RESULT_SUCCESS;
}

static inline size_t
_ccs_serialize_bin_size_ccs_object_internal(_ccs_object_internal_t *obj)
{
	...
	_ccs_object_serialize_options_t *opts)
{
	...
	case CCS_SERIALIZE_FORMAT_BINARY: {
		...
		if (obj->user_data) {
			if (obj->serialize_callback)
				CCS_VALIDATE(obj->serialize_callback(
					object, 0, NULL, &serialize_data_size,
		...
		*buffer_size +=
			_ccs_serialize_bin_size_size(serialize_data_size) +
			serialize_data_size;
		break;
	}
	default:
		...
		if (obj->user_data) {
			...
			if (obj->serialize_callback)
				CCS_VALIDATE(obj->serialize_callback(
					...
					NULL, obj->serialize_user_data));
			else if (opts->serialize_callback)
				CCS_VALIDATE(opts->serialize_callback(
					object, serialize_data_size, *buffer,
					NULL, opts->serialize_user_data));
		}
