...
_ccs_interval_include(ccs_interval_t *interval, ccs_numeric_t value)
{
	if (interval->type == CCS_NUM_FLOAT) {
		return (interval->lower_included
				...
				: interval->lower.f < value.f) &&
		       (interval->upper_included ? interval->upper.f >= value.f
						 : interval->upper.f > value.f);
	} else {
		return (interval->lower_included
				? interval->lower.i <= value.i
				: interval->lower.i < value.i) &&
		       (interval->upper_included ? interval->upper.i >= value.i
...
#define CCS_CHECK_OBJ(o, t)                                                    \
	CCS_REFUTE_MSG(                                                        \
		...
		CCS_INVALID_OBJECT,                                            \
		"Invalid CCS object '%s' == %p supplied, expected %s",         \
		#o,                                                            \
		o,                                                             \
		#t)
...
#define CCS_CHECK_ARY(c, a)                                                    \
	CCS_REFUTE_MSG(                                                        \
		(c > 0) && !(a),                                               \
		CCS_INVALID_VALUE,                                             \
		"Invalid array '%s' == %p of size '%s' == %zu supplied",       \
		#a,                                                            \
		a,                                                             \
		#c,                                                            \
		c)
...
#define CCS_REFUTE(cond, error)                                                \
	CCS_REFUTE_MSG(                                                        \
		cond,                                                          \
		error,                                                         \
		"%s: Error condition '%s' was verified",                       \
		#error,                                                        \
		#cond)
