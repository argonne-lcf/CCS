...
_ccs_deserialize_bin_ccs_tree_data(
	...
	if (data->arity) {
		data->children = (ccs_tree_t *)
			calloc(data->arity, sizeof(ccs_tree_t));
		...
		for (size_t i = 0; i < data->arity; i++) {
			...
			if (present)
				CCS_VALIDATE(_ccs_tree_deserialize(
					data->children + i, CCS_SERIALIZE_FORMAT_BINARY, version, buffer_size, buffer, &new_opts));
		}
	}
	CCS_VALIDATE(_ccs_deserialize_bin_ccs_float(
...
_ccs_deserialize_bin_tree(
	...
	_ccs_tree_data_mock_t data;
	data.children = NULL;
	CCS_VALIDATE_ERR_GOTO(res, _ccs_deserialize_bin_ccs_tree_data(
		&data, version, buffer_size, buffer, opts), end);
	CCS_VALIDATE_ERR_GOTO(res, ccs_create_tree(data.arity, data.value, &tree), end);
	CCS_VALIDATE_ERR_GOTO(res, ccs_tree_set_weight(tree, data.weight), err_tree);
	CCS_VALIDATE_ERR_GOTO(res, ccs_tree_set_bias(tree, data.bias), err_tree);
	for (size_t i = 0; i < data.arity; i++)
		if (data.children[i])
			CCS_VALIDATE_ERR_GOTO(res, ccs_tree_set_child(tree, i, data.children[i]), err_tree);
...
_ccs_tree_deserialize(
	...
	CCS_VALIDATE(_ccs_object_deserialize_user_data(
		(ccs_object_t)*tree_ret, format, version, buffer_size, buffer, opts));
	return CCS_SUCCESS;
}
