...
_ccs_serialize_bin_ccs_tree_data(
	...
	_ccs_object_serialize_options_t *opts)
{
	CCS_VALIDATE(_ccs_serialize_bin_size(data->arity, buffer_size, buffer));
	...
	CCS_VALIDATE(
		_ccs_serialize_bin_ccs_float(data->bias, buffer_size, buffer));
	CCS_VALIDATE(
...
static _ccs_tree_ops_t _ccs_tree_ops = {
	{&_ccs_tree_del, &_ccs_tree_serialize_size, &_ccs_tree_serialize}};
...
ccs_create_tree(size_t arity, ccs_datum_t value, ccs_tree_t *tree_ret)
{
	...
	CCS_REFUTE(
		value.type > CCS_DATA_TYPE_STRING,
		CCS_RESULT_ERROR_INVALID_VALUE);
	CCS_REFUTE(arity > CCS_INT_MAX - 1, CCS_RESULT_ERROR_INVALID_VALUE);
	size_t size_strs = 0;
	if (value.type == CCS_DATA_TYPE_STRING) {
		CCS_REFUTE(!value.value.s, CCS_RESULT_ERROR_INVALID_VALUE);
...
static inline ccs_result_t
_ccs_tree_update_weight(ccs_tree_t tree, size_t index, ccs_float_t weight)
{
	_ccs_tree_data_t *tree_data = tree->data;
	CCS_REFUTE(index > tree_data->arity, CCS_RESULT_ERROR_OUT_OF_BOUNDS);
...
static inline ccs_result_t
_ccs_tree_update_bias(ccs_tree_t tree, ccs_float_t bias)
{
	_ccs_tree_data_t *tree_data = tree->data;
...
{
	CCS_CHECK_OBJ(tree, CCS_OBJECT_TYPE_TREE);
	CCS_CHECK_OBJ(child, CCS_OBJECT_TYPE_TREE);
	_ccs_tree_data_t *tree_data = tree->data;

	CCS_REFUTE(index >= tree_data->arity, CCS_RESULT_ERROR_OUT_OF_BOUNDS);
	CCS_REFUTE(tree_data->children[index], CCS_RESULT_ERROR_INVALID_VALUE);
	_ccs_tree_data_t *child_data = child->data;
	CCS_REFUTE(
		child_data->parent || child_data->index,
	...
	CCS_VALIDATE_ERR_GOTO(
		err, _ccs_tree_update_weight(tree, index, weight), err_distrib);
	return CCS_RESULT_SUCCESS;
err_distrib:
	...
	size_t     *num_children_ret)
{
	...
	CCS_REFUTE(
		!children && !num_children_ret, CCS_RESULT_ERROR_INVALID_VALUE);
	_ccs_tree_data_t *data  = tree->data;
	size_t            arity = data->arity;
	if (children) {
		CCS_REFUTE(
			num_children < arity, CCS_RESULT_ERROR_INVALID_VALUE);
...
{
	CCS_CHECK_OBJ(tree, CCS_OBJECT_TYPE_TREE);
	CCS_REFUTE(weight < 0.0, CCS_RESULT_ERROR_INVALID_VALUE);
	_ccs_tree_data_t *tree_data  = tree->data;
...
{
	CCS_CHECK_OBJ(tree, CCS_OBJECT_TYPE_TREE);
	CCS_REFUTE(bias < 0.0, CCS_RESULT_ERROR_INVALID_VALUE);
	_ccs_tree_data_t *tree_data = tree->data;
	ccs_float_t       old_bias  = tree_data->bias;
	ccs_result_t      err       = CCS_RESULT_SUCCESS;
	CCS_VALIDATE_ERR_GOTO(
		err, _ccs_tree_update_bias(tree, bias), err_distrib);
	return CCS_RESULT_SUCCESS;
err_distrib:
	...
	size_t    *position_size_ret)
{
	...
	if (position)
		CCS_REFUTE(
			position_size < depth, CCS_RESULT_ERROR_INVALID_VALUE);
	...
	size_t      *num_values_ret)
{
	...
	if (values)
		CCS_REFUTE(
			num_values < depth + 1, CCS_RESULT_ERROR_INVALID_VALUE);
	if (num_values_ret)
	...
	ccs_datum_t  *values)
{
	...
	CCS_CHECK_ARY(num_values, values);
	CCS_REFUTE(
		num_values < position_size + 1, CCS_RESULT_ERROR_INVALID_VALUE);
...
ccs_tree_sample(ccs_tree_t tree, ccs_rng_t rng, size_t *index_ret)
{
	...
	CCS_VALIDATE(_ccs_tree_samples(data, rng, 1, index_ret));
	return CCS_RESULT_SUCCESS;
}
	...
	size_t    *indices)
{
	...
	_ccs_tree_data_t *data = (_ccs_tree_data_t *)(tree->data);
	CCS_VALIDATE(_ccs_tree_samples(data, rng, num_indices, indices));
	return CCS_RESULT_SUCCESS;
}
