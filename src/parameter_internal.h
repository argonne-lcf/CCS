...
#define CCS_CHECK_PARAMETER(o, t) do { \
	CCS_CHECK_OBJ(o, CCS_PARAMETER); \
	CCS_REFUTE(((_ccs_parameter_common_data_t*)(o->data))->type != (t), CCS_INVALID_PARAMETER); \
} while (0)
...
_ccs_serialize_bin_ccs_parameter_common_data(
		...
		size_t                             *buffer_size,
		char                              **buffer) {
	CCS_VALIDATE(_ccs_serialize_bin_ccs_parameter_type(data->type, buffer_size, buffer));
...
_ccs_deserialize_bin_ccs_parameter_common_data(
		...
		size_t                             *buffer_size,
		const char                        **buffer) {
	CCS_VALIDATE(_ccs_deserialize_bin_ccs_parameter_type(&data->type, buffer_size, buffer));
