...
typedef struct _ccs_random_tree_tuner_data_mock_s _ccs_random_tree_tuner_data_mock_t;
...
_ccs_deserialize_bin_size_ccs_tree_tuner_common_data(
	...
	CCS_VALIDATE(_ccs_tree_space_deserialize(
		&data->tree_space, CCS_SERIALIZE_FORMAT_BINARY, version, buffer_size, buffer, opts));
...
_ccs_deserialize_bin_ccs_random_tree_tuner_data(
	...
	mem = (uintptr_t)calloc(
		(data->size_history + data->size_optimums), sizeof(ccs_tree_evaluation_t));
	CCS_REFUTE(!mem, CCS_OUT_OF_MEMORY);

	data->history = (ccs_tree_evaluation_t *)mem;
	mem += data->size_history * sizeof(ccs_tree_evaluation_t);
	data->optimums = (ccs_tree_evaluation_t *)mem;
...
typedef struct _ccs_random_tree_tuner_data_clone_s _ccs_random_tree_tuner_data_clone_t;
...
_ccs_deserialize_bin_random_tree_tuner(
		...
		const char                        **buffer,
		_ccs_object_deserialize_options_t  *opts) {
	_ccs_random_tree_tuner_data_mock_t data = {{(ccs_tree_tuner_type_t)0, NULL, NULL, NULL}, 0, 0, NULL, NULL};
	_ccs_random_tree_tuner_data_clone_t *odata = NULL;
	ccs_error_t res = CCS_SUCCESS;
	CCS_VALIDATE_ERR_GOTO(res, _ccs_deserialize_bin_ccs_random_tree_tuner_data(
		&data, version, buffer_size, buffer, opts), evaluations);
	CCS_VALIDATE_ERR_GOTO(res, ccs_create_random_tree_tuner(
		data.common_data.name, data.common_data.tree_space, data.common_data.objective_space,
...
typedef struct _ccs_user_defined_tree_tuner_data_mock_s _ccs_user_defined_tree_tuner_data_mock_t;
...
_ccs_deserialize_bin_user_defined_tree_tuner(
		...
		const char                        **buffer,
		_ccs_object_deserialize_options_t  *opts) {
	_ccs_user_defined_tree_tuner_data_mock_t data = {{{(ccs_tree_tuner_type_t)0, NULL, NULL, NULL}, 0, 0, NULL, NULL}, {0, NULL}};
	ccs_user_defined_tree_tuner_vector_t *vector = (ccs_user_defined_tree_tuner_vector_t *)opts->vector;
	ccs_error_t res = CCS_SUCCESS;
	CCS_VALIDATE_ERR_GOTO(res, _ccs_deserialize_bin_ccs_user_defined_tree_tuner_data(
		&data, version, buffer_size, buffer, opts), end);
	CCS_VALIDATE_ERR_GOTO(res, ccs_create_user_defined_tree_tuner(
		data.base_data.common_data.name, data.base_data.common_data.tree_space, data.base_data.common_data.objective_space,
...
_ccs_deserialize_bin_tree_tuner(
	...
	CCS_VALIDATE(_ccs_peek_bin_ccs_tree_tuner_type(
	...
	case CCS_TREE_TUNER_RANDOM:
		CCS_VALIDATE_ERR_GOTO(res, _ccs_deserialize_bin_random_tree_tuner(
			tuner_ret, version, buffer_size, buffer, &new_opts), end);
		break;
	case CCS_TREE_TUNER_USER_DEFINED:
		CCS_VALIDATE_ERR_GOTO(res, _ccs_deserialize_bin_user_defined_tree_tuner(
			tuner_ret, version, buffer_size, buffer, &new_opts), end);
		break;
	default:
