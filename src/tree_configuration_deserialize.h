...
_ccs_deserialize_bin_tree_configuration_data(
	...
	if (data->position_size) {
		data->position = (size_t *)malloc(data->position_size * sizeof(size_t));
...
_ccs_deserialize_bin_tree_configuration(
	...
	CCS_REFUTE(obj.type != CCS_TREE_CONFIGURATION, CCS_INVALID_TYPE);
	_ccs_tree_configuration_data_t data= { NULL, 0, NULL };
	ccs_error_t res = CCS_SUCCESS;
	CCS_VALIDATE_ERR_GOTO(res, _ccs_deserialize_bin_tree_configuration_data(
		&data, version, buffer_size, buffer), end);

	CCS_VALIDATE_ERR_GOTO(res, ccs_map_get(
		opts->handle_map, ccs_object(data.tree_space), &d), end);
	CCS_REFUTE_ERR_GOTO(res, d.type != CCS_OBJECT, CCS_INVALID_HANDLE, end);
	tree_space = (ccs_tree_space_t)(d.value.o);

	CCS_VALIDATE_ERR_GOTO(res, ccs_create_tree_configuration(
		tree_space, data.position_size, data.position, &configuration), end);
