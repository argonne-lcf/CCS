...
_ccs_object_serialize_options(
	...
	while (opt != CCS_SERIALIZE_OPTION_END) {
		...
		case CCS_SERIALIZE_OPTION_CALLBACK:
			opts->serialize_callback = va_arg(args, ccs_object_serialize_callback_t);
...
_ccs_object_serialize_size_with_opts(
	...
	*buffer_size = _ccs_serialize_header_size(format);
	CCS_VALIDATE(obj->ops->serialize_size(object, format, buffer_size));
	CCS_VALIDATE(_ccs_object_serialize_user_data_size(object, format, buffer_size, opts));
	return CCS_SUCCESS;
}
...
_ccs_object_serialize_file_descriptor(
	...
	if (!pstate || !pstate->base) {
		...
		pstate->fd = fd;
		CCS_VALIDATE_ERR_GOTO(res, _ccs_object_serialize_memory_with_opts(
			object, format, pstate->buffer_size, pstate->buffer, &opts), err_fd_buffer);
	}
	do {
		ssize_t count;
...
_ccs_object_deserialize_file_descriptor(
	...
	if (!non_blocking || !pstate->base_size) {
		...
		CCS_VALIDATE_ERR_GOTO(res, _ccs_deserialize_header(
			format, &pstate->buffer_size, (const char **)&pstate->buffer, &object_size, &pstate->version), err_fd_buffer);
