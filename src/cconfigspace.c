	...
	_ccs_object_serialize_options_t *opts)
{
	...
	while (opt != CCS_SERIALIZE_OPTION_END) {
		...
		default:
			CCS_RAISE(
				CCS_INVALID_VALUE,
				"Unsupported serialization option: %d",
				opt);
		}
	...
	va_list                args)
{
	...
	CCS_REFUTE_ERR_GOTO(
		res,
		msync(buffer, buffer_size, MS_SYNC) == -1,
		CCS_SYSTEM_ERROR,
		err_file_map);
err_file_map:
	...
	...)
{
	...
	case CCS_SERIALIZE_OPERATION_SIZE:
		CCS_VALIDATE_ERR_GOTO(
			res,
			_ccs_object_serialize_size(object, format, args),
			end);
		break;
	case CCS_SERIALIZE_OPERATION_MEMORY:
		CCS_VALIDATE_ERR_GOTO(
			res,
			_ccs_object_serialize_memory(object, format, args),
			end);
		break;
	case CCS_SERIALIZE_OPERATION_FILE:
		CCS_VALIDATE_ERR_GOTO(
			res,
			_ccs_object_serialize_file(object, format, args),
			end);
		break;
	...
	default:
		CCS_RAISE_ERR_GOTO(
			...
			"Unsupported serialize operation: %d",
			operation);
	}
end:
	va_end(args);
	...
	_ccs_object_deserialize_options_t *opts)
{
	...
	while (opt != CCS_DESERIALIZE_OPTION_END) {
		...
		default:
			CCS_RAISE(
				CCS_INVALID_VALUE,
				"Unsupported deserialization option: %d",
				opt);
		}
	...
	va_list                   args)
{
	...
	size_t                            size;
	_ccs_object_deserialize_options_t opts = {
		NULL, CCS_TRUE, NULL, NULL, NULL, NULL, NULL};
	...
	...)
{
	...
	default:
		CCS_RAISE_ERR_GOTO(
			...
			"Unsupported deserialize operation: %d",
			operation);
	}
end:
	va_end(args);
