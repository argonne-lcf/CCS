...
#define utarray_oom()                                                          \
	{                                                                      \
		...
		CCS_RAISE_ERR_GOTO(                                            \
			err, CCS_OUT_OF_MEMORY, errarrays,                     \
...
#define utarray_oom()                                                          \
	{                                                                      \
		CCS_RAISE_ERR_GOTO(                                            \
			err, CCS_OUT_OF_MEMORY, errordistrib_wrapper,          \
			"Out of memory to allocate array");                    \
	}
	...
	ccs_distribution_t        distribution)
{
	...
	if (distribution) {
		CCS_VALIDATE_ERR_GOTO(
			...
			errorparameter);
		CCS_REFUTE_ERR_GOTO(
			err, dimension != 1, CCS_INVALID_DISTRIBUTION,
			errorparameter);
		CCS_VALIDATE_ERR_GOTO(
