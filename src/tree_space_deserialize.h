...
_ccs_deserialize_bin_ccs_tree_space_common_data(
	...
	CCS_VALIDATE(_ccs_rng_deserialize(
		&data->rng, CCS_SERIALIZE_FORMAT_BINARY, version, buffer_size, buffer, &new_opts));
	CCS_VALIDATE(_ccs_tree_deserialize(
		&data->tree, CCS_SERIALIZE_FORMAT_BINARY, version, buffer_size, buffer, &new_opts));
	return CCS_SUCCESS;
}
...
_ccs_deserialize_bin_tree_space_static(
	...
	CCS_VALIDATE_ERR_GOTO(res, ccs_create_static_tree_space(
		data.name, data.tree, tree_space_ret), end);
end:
	if (data.rng)
...
typedef struct _ccs_tree_space_dynamic_data_mock_s _ccs_tree_space_dynamic_data_mock_t;
...
_ccs_deserialize_bin_tree_space_dynamic(
	...
	ccs_error_t res = CCS_SUCCESS;
	CCS_VALIDATE_ERR_GOTO(res, _ccs_deserialize_bin_ccs_tree_space_common_data(
		&data.common_data, version, buffer_size, buffer, opts), end);
	CCS_VALIDATE_ERR_GOTO(res, ccs_create_dynamic_tree_space(
		data.common_data.name, data.common_data.tree, vector, opts->data, tree_space_ret), end);
	CCS_VALIDATE_ERR_GOTO(res, ccs_tree_space_set_rng(
		*tree_space_ret, data.common_data.rng), tree_space);
	if (vector->deserialize_state)
		CCS_VALIDATE_ERR_GOTO(res, vector->deserialize_state(
			*tree_space_ret, data.blob.sz, data.blob.blob), tree_space);
	goto end;
tree_space:
...
_ccs_deserialize_bin_tree_space(
	...
	CCS_VALIDATE(_ccs_peek_bin_ccs_tree_space_type(
		&stype, buffer_size, buffer));
	switch (stype) {
	...
	case CCS_TREE_SPACE_TYPE_DYNAMIC:
		...
		break;
	default:
		CCS_RAISE(CCS_UNSUPPORTED_OPERATION, "Unsupported tree space type: %d", stype);
	}
...
_ccs_tree_space_deserialize(
	...
	CCS_VALIDATE(_ccs_object_deserialize_user_data(
		(ccs_object_t)*tree_space_ret, format, version, buffer_size, buffer, opts));
	return CCS_SUCCESS;
}
