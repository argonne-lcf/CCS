...
class TestDistribution(unittest.TestCase):
  ...
  def test_create_roulette(self):
    ...
    self.assertEqual( ccs.ccs_object_type.DISTRIBUTION, d.object_type )
    self.assertEqual( ccs.ccs_distribution_type.ROULETTE, d.type )
    self.assertEqual( ccs.ccs_numeric_type.INT, d.data_type )
    ...
    i = d.bounds
    self.assertEqual( ccs.ccs_numeric_type.INT, i.type)
  ...
  def test_create_normal(self):
    d = ccs.NormalDistribution()
    self.assertEqual( ccs.ccs_object_type.DISTRIBUTION, d.object_type )
    self.assertEqual( ccs.ccs_distribution_type.NORMAL, d.type )
    self.assertEqual( ccs.ccs_numeric_type.FLOAT, d.data_type )
    self.assertEqual( ccs.ccs_scale_type.LINEAR, d.scale )
    self.assertEqual( 1, d.dimension )
    self.assertEqual( 0.0, d.mu )
    self.assertEqual( 1.0, d.sigma )
    i = d.bounds
    self.assertEqual( ccs.ccs_numeric_type.FLOAT, i.type )
  ...
  def test_create_uniform(self):
    d = ccs.UniformDistribution()
    self.assertEqual( ccs.ccs_object_type.DISTRIBUTION, d.object_type )
    self.assertEqual( ccs.ccs_distribution_type.UNIFORM, d.type )
    self.assertEqual( ccs.ccs_numeric_type.FLOAT, d.data_type )
    self.assertEqual( ccs.ccs_scale_type.LINEAR, d.scale )
    self.assertEqual( 1, d.dimension )
    i = d.bounds
  ...
  def test_create_mixture(self):
    ...
    self.assertEqual( d.object_type, ccs.ccs_object_type.DISTRIBUTION )
    self.assertEqual( d.type, ccs.ccs_distribution_type.MIXTURE )
    self.assertEqual( d.data_types, [ccs.ccs_numeric_type.FLOAT] )
  ...
  def test_create_multivariate(self):
    ...
    self.assertEqual( d.object_type, ccs.ccs_object_type.DISTRIBUTION )
    self.assertEqual( d.type, ccs.ccs_distribution_type.MULTIVARIATE )
    self.assertEqual( d.data_types, [ccs.ccs_numeric_type.FLOAT, ccs.ccs_numeric_type.INT] )
    self.assertEqual( [x.handle.value for x in d.distributions], [x.handle.value for x in distributions] )
    d2 = ccs.Object.from_handle(d.handle)
    self.assertEqual( d.__class__, d2.__class__ )
  ...
  def test_mixture_multidim(self):
    ...
    self.assertEqual( d2.object_type, ccs.ccs_object_type.DISTRIBUTION )
    self.assertEqual( d2.type, ccs.ccs_distribution_type.MIXTURE )
    self.assertEqual( d2.data_types, [ccs.ccs_numeric_type.FLOAT, ccs.ccs_numeric_type.INT] )
