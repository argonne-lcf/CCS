...
class TestConfigurationSpace(unittest.TestCase):
  ...
  def test_conditions(self):
    ...
    e2 = ccs.Expression(t = ccs.ExpressionType.LESS, nodes = [h3, 0.0])
    cs.set_condition(h1, e2)
    e3 = ccs.Expression(t = ccs.ExpressionType.LESS, nodes = [h1, 0.0])
  ...
  def test_omp(self):
    ...
    cond0 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p1, '#pragma omp #P2'])
    cond1 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p1, '#pragma omp target teams distribute #P2'])
    cond2 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p1, '#pragma omp target teams distribute #P4'])
    cond3 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p1, '#pragma omp #P3'])

    cond4 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p2, 'parallel for #P3'])
    cond5 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p2, 'parallel for #P5'])
    cond6 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p2, 'parallel for #P6'])

    cond7 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p4, 'dist_schedule(static, #P8)'])

    cond8 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p5, 'schedule(#P7)'])
    cond9 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p5, 'schedule(#P7,#P8)'])

    cond10 = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p6, 'numthreads(#P9)'])

    cs.set_condition(p2, ccs.Expression(t = ccs.ExpressionType.OR, nodes = [cond0, cond1]))
    cs.set_condition(p4, cond2)
    cs.set_condition(p3, ccs.Expression(t = ccs.ExpressionType.OR, nodes = [cond3, cond4]))
    cs.set_condition(p5, cond5)
    cs.set_condition(p6, cond6)
    cs.set_condition(p7, ccs.Expression(t = ccs.ExpressionType.OR, nodes = [cond8, cond9]))
    cs.set_condition(p8, ccs.Expression(t = ccs.ExpressionType.OR, nodes = [cond7, cond9]))
    cs.set_condition(p9, cond10)

    forbiddena = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p1, '#pragma omp #P2'])
    forbiddenb = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p2, ' '])
    forbidden0 = ccs.Expression(t = ccs.ExpressionType.AND, nodes = [forbiddena, forbiddenb])

    forbiddenc = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p1, '#pragma omp #P3'])
    forbiddend = ccs.Expression(t = ccs.ExpressionType.EQUAL, nodes = [p3, ' '])
    forbidden1 = ccs.Expression(t = ccs.ExpressionType.AND, nodes = [forbiddenc, forbiddend])
