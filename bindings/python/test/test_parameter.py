...
class TestParameter(unittest.TestCase):
  ...
  def test_discrete(self):
    ...
    h = ccs.DiscreteParameter(values = values)
    self.assertEqual( ccs.ccs_object_type.PARAMETER, h.object_type )
    self.assertEqual( ccs.ccs_parameter_type.DISCRETE, h.type )
  ...
  def test_ordinal(self):
    ...
    h = ccs.OrdinalParameter(values = values)
    self.assertEqual( ccs.ccs_object_type.PARAMETER, h.object_type )
    self.assertEqual( ccs.ccs_parameter_type.ORDINAL, h.type )
    self.assertTrue( h.name[:5] == "param" )
    self.assertIsNone( h.user_data )
    self.assertEqual( "foo", h.default_value )
    self.assertEqual( ccs.ccs_distribution_type.UNIFORM, h.default_distribution.type )
  ...
  def test_categorical(self):
    ...
    h = ccs.CategoricalParameter(values = values)
    self.assertEqual( ccs.ccs_object_type.PARAMETER, h.object_type )
    self.assertEqual( ccs.ccs_parameter_type.CATEGORICAL, h.type )
    self.assertTrue( h.name[:5] == "param" )
    self.assertIsNone( h.user_data )
  ...
  def test_numerical(self):
    h = ccs.NumericalParameter()
    self.assertEqual( ccs.ccs_object_type.PARAMETER, h.object_type )
    self.assertEqual( ccs.ccs_parameter_type.NUMERICAL, h.type )
    self.assertTrue( h.name[:5] == "param" )
    self.assertIsNone( h.user_data )
    self.assertEqual( 0.0, h.default_value )
    self.assertEqual( ccs.ccs_distribution_type.UNIFORM, h.default_distribution.type )
    self.assertEqual( ccs.ccs_numeric_type.FLOAT, h.data_type )
  ...
  def test_numerical_int(self):
    ...
    self.assertEqual( ccs.ccs_distribution_type.UNIFORM, h.default_distribution.type )
    self.assertEqual( ccs.ccs_numeric_type.INT, h.data_type )
  ...
  def test_string(self):
    ...
    self.assertEqual( ccs.ccs_object_type.PARAMETER, h.object_type )
    self.assertEqual( ccs.ccs_parameter_type.STRING, h.type )
    self.assertTrue( h.name[:5] == "param" )
    self.assertIsNone( h.user_data )
