...
from .base import Object, Error, ccs_result, ccs_int, ccs_float, ccs_bool, ccs_rng, ccs_distribution, ccs_numeric_type, ccs_numeric, CEnumeration, _ccs_get_function, ccs_false, ccs_true
...
class Distribution(Object):
  ...
  def from_handle(cls, handle, retain = True, auto_release = True):
    ...
    if v == ccs_distribution_type.UNIFORM:
      ...
      raise Error(ccs_result(ccs_result.ERROR_INVALID_DISTRIBUTION))
  ...
  def sample(self, rng):
    ...
    if dim == 1:
      ...
      return [ v[i].i if self.data_types[i] == ccs_numeric_type.INT else v[i].f for i in range(dim) ]
  ...
  def samples(self, rng, count):
    ...
    if dim == 1:
      ...
      Error.check(res)
      return [ [v[j][i].i if self.data_types[i] == ccs_numeric_type.INT else v[j][i].f for i in range(dim) ] for j in range(count) ]
...
class UniformDistribution(Distribution):
  def __init__(self, handle = None, retain = False, auto_release = True,
               data_type = ccs_numeric_type.FLOAT, lower = 0.0, upper = 1.0, scale = ccs_scale_type.LINEAR, quantization = 0.0):
    if handle is None:
  ...
  def int(cls, lower, upper, scale = ccs_scale_type.LINEAR, quantization = 0):
    return cls(data_type = ccs_numeric_type.INT, lower = lower, upper = upper, scale = scale, quantization = quantization)

  @classmethod
  def float(cls, lower, upper, scale = ccs_scale_type.LINEAR, quantization = 0.0):
    return cls(data_type = ccs_numeric_type.FLOAT, lower = lower, upper = upper, scale = scale, quantization = quantization)
...
class NormalDistribution(Distribution):
  def __init__(self, handle = None, retain = False, auto_release = True,
               data_type = ccs_numeric_type.FLOAT, mu = 0.0, sigma = 1.0, scale = ccs_scale_type.LINEAR, quantization = 0.0):
    if handle is None:
      handle = ccs_distribution(0)
      if data_type == ccs_numeric_type.FLOAT:
  ...
  def int(cls, mu, sigma, scale = ccs_scale_type.LINEAR, quantization = 0):
    return cls(data_type = ccs_numeric_type.INT, mu = mu, sigma = sigma, scale = scale, quantization = quantization)

  @classmethod
  def float(cls, mu, sigma, scale = ccs_scale_type.LINEAR, quantization = 0.0):
    return cls(data_type = ccs_numeric_type.FLOAT, mu = mu, sigma = sigma, scale = scale, quantization = quantization)
