...
ccs_tree_tuner_get_optimums = _ccs_get_function("ccs_tree_tuner_get_optimums", [ccs_tree_tuner, ct.c_size_t, ct.POINTER(ccs_tree_evaluation), ct.POINTER(ct.c_size_t)])
...
class TreeTuner(Object):
  ...
  def num_optimums(self):
    v = ct.c_size_t()
    res = ccs_tree_tuner_get_optimums(self.handle, 0, None, ct.byref(v))
    Error.check(res)
    return v.value
  ...
  def optimums(self):
    ...
    v = (ccs_tree_evaluation * count)()
    res = ccs_tree_tuner_get_optimums(self.handle, count, v, None)
    Error.check(res)
    return [TreeEvaluation.from_handle(ccs_tree_evaluation(x)) for x in v]
...
ccs_user_defined_tree_tuner_del_type = ct.CFUNCTYPE(ccs_error, ccs_tree_tuner)
ccs_user_defined_tree_tuner_ask_type = ct.CFUNCTYPE(ccs_error, ccs_tree_tuner, ct.c_size_t, ct.POINTER(ccs_tree_configuration), ct.POINTER(ct.c_size_t))
ccs_user_defined_tree_tuner_tell_type = ct.CFUNCTYPE(ccs_error, ccs_tree_tuner, ct.c_size_t, ct.POINTER(ccs_tree_evaluation))
ccs_user_defined_tree_tuner_get_optimums_type = ct.CFUNCTYPE(ccs_error, ccs_tree_tuner, ct.c_size_t, ct.POINTER(ccs_tree_evaluation), ct.POINTER(ct.c_size_t))
ccs_user_defined_tree_tuner_get_history_type = ct.CFUNCTYPE(ccs_error, ccs_tree_tuner, ct.c_size_t, ct.POINTER(ccs_tree_evaluation), ct.POINTER(ct.c_size_t))
ccs_user_defined_tree_tuner_suggest_type = ct.CFUNCTYPE(ccs_error, ccs_tree_tuner, ct.POINTER(ccs_tree_configuration))
ccs_user_defined_tree_tuner_serialize_type = ct.CFUNCTYPE(ccs_error, ccs_tree_tuner, ct.c_size_t, ct.c_void_p, ct.POINTER(ct.c_size_t))
ccs_user_defined_tree_tuner_deserialize_type = ct.CFUNCTYPE(ccs_error, ccs_tree_tuner, ct.c_size_t, ct.POINTER(ccs_tree_evaluation), ct.c_size_t, ct.POINTER(ccs_tree_evaluation), ct.c_size_t, ct.c_void_p)
...
class ccs_user_defined_tree_tuner_vector(ct.Structure):
  _fields_ = [
    ...
    ('get_optimums', ccs_user_defined_tree_tuner_get_optimums_type),
...
def _wrap_user_defined_tree_tuner_callbacks(delete, ask, tell, get_optimums, get_history, suggest, serialize, deserialize):
    ...
    def deserialize_wrapper(tun, size_history, p_history, num_optimums, p_optimums, state_size, p_state):
      try:
        ...
        if p_o.value is None:
          ...
          optimums = [TreeEvaluation.from_handle(ccs_tree_evaluation(p_o[i])) for i in range(num_optimums)]
        if p_s.value is None:
          state = None
        else:
          state = ct.cast(p_s, POINTER(c_byte * state_size))
        deserialize(TreeTuner.from_handle(tun), history, optimums, state)
        return ccs_error.SUCCESS
      except Exception as e:
        return Error.set_error(e)
  else:
  ...
  return (delete_wrapper,
          ...
          ccs_user_defined_tree_tuner_get_optimums_type(get_optimums_wrapper),
  ...
  def __init__(self, handle = None, retain = False, auto_release = True,
    ...
    if handle is None:
      ...
      (delete_wrapper,
       ...
       deserialize_wrapper_func) = _wrap_user_defined_tree_tuner_callbacks(delete, ask, tell, get_optimums, get_history, suggest, serialize, deserialize)
      handle = ccs_tree_tuner()
