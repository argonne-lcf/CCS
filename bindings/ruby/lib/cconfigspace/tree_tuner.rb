module CCS
  ...
  class TreeTuner < Object
	...
	RandomTreeTuner
      when :CCS_TREE_TUNER_USER_DEFINED
        ...
        raise CCSError, :CCS_INVALID_TREE_TUNER
    ...
    def num_optimums
      p_count = MemoryPointer::new(:size_t)
      CCS.error_check CCS.ccs_tree_tuner_get_optimums(@handle, 0, nil, p_count)
      return p_count.read_size_t
    end
    ...
    def optimums
      count = num_optimums
      p_evals = MemoryPointer::new(:ccs_tree_evaluation_t, count)
      CCS.error_check CCS.ccs_tree_tuner_get_optimums(@handle, count, p_evals, nil)
      count.times.collect { |i| TreeEvaluation::from_handle(p_evals[i].read_pointer) }
    end
  ...
  class RandomTreeTuner < TreeTuner
    def initialize(handle = nil, retain: false, auto_release: true,
                   name: nil, tree_space: nil, objective_space: nil)
      if handle
  ...
  class UserDefinedTreeTunerVector < FFI::Struct
    layout :del, :ccs_user_defined_tree_tuner_del,
           ...
           :get_optimums, :ccs_user_defined_tree_tuner_get_optimums,
  ...
  def self.wrap_user_defined_tree_tuner_callbacks(del, ask, tell, get_optimums, get_history, suggest, serialize, deserialize)
    ...
    get_optimumswrapper = lambda { |tun, count, p_evaluations, p_count|
      begin
        optimums = get_optimums.call(TreeTuner.from_handle(tun))
      ...
      if deserialize
        lambda { |tun, history_size, p_history, num_optimums, p_optimums, state_size, p_state|
          begin
            ...
            optimums = p_optimums.null? ? [] : num_optimums.times.collect { |i| TreeEvaluation::from_handle(p_optimums.get_pointer(i*8)) }
    ...
    def initialize(handle = nil, retain: false, auto_release: true,
      ...
      if handle
        ...
        delwrapper, askwrapper, tellwrapper, get_optimumswrapper, get_historywrapper, suggestwrapper, serializewrapper, deserializewrapper =
          CCS.wrap_user_defined_tree_tuner_callbacks(del, ask, tell, get_optimums, get_history, suggest, serialize, deserialize)
